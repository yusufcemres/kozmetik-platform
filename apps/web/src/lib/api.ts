...
export async function apiFetch<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      // 2026-05-17 — cache:'no-store' global olarak ekleyince Next.js static
      // generation 60s timeout oldu (/onerilerimiz). Geri çekildi. Cache kontrolü
      // sayfa-level `export const revalidate = N` ile yapılır.

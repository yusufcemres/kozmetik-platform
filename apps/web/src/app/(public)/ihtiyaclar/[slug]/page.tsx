  ...
  return (
    <>
      ...
      <article className="curator-section max-w-[1400px] mx-auto">
        {/* Breadcrumb */}
        <nav className="text-outline mb-4 flex items-center gap-2 text-xs uppercase tracking-widest">
        ...
        <div className="curator-card p-4 md:p-5 mb-6">
          ...
          <h1 className="text-2xl md:text-3xl headline-tight text-on-surface mb-1">
            {need.need_name}
          </h1>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-1">
                        <div className="flex-1 min-w-0">
                          {m.ingredient ? (
                            <Link
                              href={`/icerikler/${m.ingredient.ingredient_slug}`}
                              className="text-[11px] font-medium text-on-surface hover:text-primary transition-colors block truncate leading-tight"
                              title={m.ingredient.inci_name}
                            >
                              {m.ingredient.inci_name}
                            </Link>
                          ) : (
                          ...
                          {m.ingredient?.common_name && (
                            <p className="text-[9px] text-outline truncate" title={m.ingredient.common_name}>
                              {m.ingredient.common_name}
                            </p>
                          )}
                        </div>
                        <span className={`text-[11px] font-bold shrink-0 tabular-nums ${getScoreColor(score)}`}>
                          %{score}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 mt-1 flex-wrap">
                        <span className={`label-caps text-[8px] px-1 py-0 rounded-sm ${effect.color}`}>
                          {effect.label}
                        </span>
                        {m.ingredient?.ingredient_group && (
                          <span className="label-caps text-[8px] text-outline truncate">
                            {m.ingredient.ingredient_group}
                          </span>
                        )}
                      </div>
        ...
        <section className="mb-6">
          <h2 className="text-lg font-bold text-on-surface mb-2">Kozmetik Öneriler</h2>
          {topScores.length > 0 ? (
            <ListModal
                ...
                {topScores.slice(0, 6).map((score) => {
                  ...
                  return (
                    <Link
                      key={score.product_need_score_id}
                      href={`/urunler/${product.product_slug}`}
                      className="curator-card overflow-hidden group snap-start shrink-0 w-[160px] sm:w-[180px]"
                    >
                      ...
                      <div className="p-2">
                        ...
                        <p className="text-[11px] font-medium text-on-surface line-clamp-2 tracking-tight leading-tight">
                          {product.product_name}
                        </p>
                        <div className="mt-1 flex items-center gap-1">
                          ...
                          <span className={`text-[10px] font-bold tabular-nums ${getScoreColor(compat)}`}>
                            %{compat}
                          </span>
                        </div>
                      </div>
                  ...
                  >
                    <div className="aspect-square bg-surface-container-low flex items-center justify-center overflow-hidden relative">
                      {primaryImg ? (
                        ...
                        <span className="material-icon text-outline-variant" style={{ fontSize: '32px' }} aria-hidden="true">medication</span>
                      )}
                    </div>
                    <div className="p-2">
                      {product.brand && (
                        <p className="label-caps text-outline text-[9px] truncate">{product.brand.brand_name}</p>
                      )}
                      <p className="text-[11px] font-medium text-on-surface line-clamp-2 tracking-tight leading-tight">
                        {product.product_name}
                      </p>
        ...
        {needInteractions.length > 0 && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-on-surface mb-1 flex items-center gap-2">
              <span className="material-icon text-score-medium text-[18px]" aria-hidden="true">sync_alt</span>
              Dikkat Edilmesi Gerekenler
            </h2>
            <p className="text-[11px] text-on-surface-variant mb-2">
              Bu ihtiyacın etken bileşenlerinin bilinen etkileşimleri.
            </p>
        ...
        <section className="mb-6">
          ...
          <div className="space-y-1.5">
            {faq.map((item, i) => (
              <details key={i} className="group curator-card px-3 py-2">
                <summary className="cursor-pointer list-none [&::-webkit-details-marker]:hidden flex items-start gap-1.5">
                  <span
                  ...
                  <span className="flex-1 text-xs font-semibold text-on-surface leading-snug">{item.q}</span>
                  <span
                ...
                <p className="text-[11px] text-on-surface-variant leading-relaxed mt-2 pt-2 border-t border-outline-variant/15 ml-5">
                  {item.a}
                </p>
              </details>
            ))}
        ...
        {!hasSkinProfileCookie() && (
          <div className="curator-card p-4 text-center mb-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="text-left sm:flex-1">
              <h3 className="text-sm font-semibold text-on-surface">
                Bu ihtiyaç seninle ilgili mi?
              </h3>
              <p className="text-[11px] text-on-surface-variant">
                Cilt profilini oluştur, her üründe sana özel uyum skoru gör.
              </p>
            </div>
            <Link
              href="/profilim"
              className="curator-btn-primary text-[10px] px-4 py-2 inline-block shrink-0"
            >
              PROFİLİMİ OLUŞTUR
            </Link>
          </div>

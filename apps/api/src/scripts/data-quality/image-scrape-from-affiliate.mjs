/**
 * Affiliate URL'i olan ama gorseli olmayan canli urunler icin:
 * Trendyol/HB sayfasinda og:image meta tagini cek, product_images'a kaydet.
 */
...
console.log(`Image scrape targets: ${targets.length}\n`);
...
for (let i = 0; i < targets.length; i += PARALLEL) {
  ...
  const results = await Promise.allSettled(chunk.map(async (p) => {
    ...
    try {
      ...
      return { id: p.product_id, found: true, platform: p.platform };
    } catch (e) {
      return { id: p.product_id, found: false, err: e.message };
    }
  }));
  for (const r of results) {
    if (r.status === 'fulfilled' && r.value.found) { success++; if (success % 20 === 0) console.log(`  ${success} images added`); }
    else failed++;
  }
  await sleep(200);
}
